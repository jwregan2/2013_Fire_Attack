...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=\columnwidth]{Figures/Water_Distribution/ADD_Bins.png}
...
Research on nozzles and water streams started as far back as 1897 \cite{CHICAGO_TRIBUNE:1939:a}. Much of the work focused on visual obeservation of distance \hl{NEED TO ADD ORIGINAL SOURCE OF HOSE TESTS} and calculated flow rate based on pressure. Without the aid of modern protective equipment early fire service nozzles needed to be designed for reach, providing protection for firefighters by keeping distance between them and the fire. In addition, more reach meant more ability to suppress fires on upper floors without having to drag hoselines through the building. Even adjustable fog nozzles have been in the fire service over 100yrs \cite{PALMER:1878} and research has focused on reach, flow and paterns. 

\hl{Need to find John R. Freedman's original work on horizontal and vertial distance.} Items to highlight 1)32~$^\circ$ Angle, 2)Elevation formula 3)

Work done by the some of the major nozzle manufacturers indicates modern nozzles using a straight stream or smooth boore have an effective reach of anywhere from 45~ft to 145~ft at a 32~$^\circ$ angle.  \cite{TFT_Reach:1994} \cite{Elkhart_Chief_Flow} \cite{AKRON_Reach:2017}. Effecitve reach is defined as the distance the stream travels before it visually break up. These same nozzles, designed for stream reach are also used for residential interior fire attack where the largest dimension of any ony room is often below 25~ft. During an interior attack on a residential structure, the hose stream will impact an obstruction long before it exceeds its effective reach. After impacting an obstruction the stream follows the principals of fluid motion. Even adjustible nozzles set to a narrow fog (30~$^\circ$) are cited with having an effective reach of 40~ft to 50~ft \cite{Elkhart_Chief_Flow}, which results in a similar challange during residential structure fires of obstructions to the stream deteriming the dispersion. 

\subsubsection{Hose Stream Dispersion}

As discussed in the reach section, hose streams used for interior structural firefighting will impact a surface before their effective reach is achieved. The measurment of what happens to the stream once it impacts a solid obstruction (dispersion), is difficult to measure and quantify. Additionally visual observasion gained form experience using hose streams on residential fires are nearly impossible due to the smoke within the structure. Work done by many of the nozzle manufactures indicate the velocity in excess of 80~ft/s \cite{NBFU:EffectiveReach}. That velocity combined with the mass of water being 8.3~lbs/gal \cite{SFPEHandbookPurser} results in a signiciicant amount of momentum behind the water. Once it impacts a surface, some of the momentum is absorbed while the remainder is transmitted in different directions. 

The direction the stream breaks into was dependant on the angle of impact. When the angle of impact was 90 degrees as shown in Figure X the stream spread out in 360~Degrees. As the angle decreased towards 0~Degrees with the surface more and more of the water maintained momentem in the direction of the initial stream. Figure X shows the change in water dispersion as the angle decreases. \hl{Add figures from keith, smoothbore with arrows showing where the water goes}
