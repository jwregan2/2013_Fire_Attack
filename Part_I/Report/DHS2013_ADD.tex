...
The dispersion of stream was dependent on the angle of impact. When the angle of impact was 90 degrees as shown in Figure~\ref{fig:90DegreeImpact} the stream spread out in 360$^{\circ}$. As the angle decreased towards 0$^{\circ}$ with the surface, more and more of the water maintained momentum in the direction of the initial stream. Figure~\ref{fig:SS_Dispersion_Arrow} shows the change in water dispersion as the angle decreases. 
...
When the stream impacted the surface at an angle, there was limited to no ricochet off the ceiling. The stream dispersed into a fan shape and continued in the direction of travel. Once it struck the back wall, the momentum was translated down the back wall. Figure \ref{fig:Mid_Ceiling_Dispersion} shows this from below for both a smooth bore, straight stream. Figure \ref{fig:Fog_Mid_Ceiling_Dispersion} shows the dispersion from a fog stream.
...
The angle at which the stream impacted the surface was shown to have the most effect on distribution in the room. Figure \ref{fig:Interior_Varying_Nozzle_Direction_SB_Fixed_Pattern} and Figure \ref{fig:Exterior_First_Floor_Varying_Nozzle_Directions_SB_Fixed_Pattern} for the smoothbore interior and smoothbore exterior attack, respectivily, show varying the angle of the nozzle impact with a surface can mor effectivly distribute the water in the compartment. Rather than nozzle movement in the left and right directions during applicaiton, varying the angle of the nozzle from a steep to a shallow angle and back would provide the best distribution in the rooom.  

\subsubsection{Bale Position}
Another option which was seen to impact the distribution for a smooth bore nozzle was the bale position. The smoth bore nozzle often incorperates a ball valve to turn the nozzle on and off. If this valve is opened completely, the opening in the ball creates a smooth transition between the ball and the surrounding housing. If the bale is opened less than full, the water traveling through the valve becomes turbulant which effects the hose stream pattern and reach. Figure \ref{fig:Bale Position} shows how opening the turbulant pattern created when the bale is half open creates a better distribution within the compartment as the velocity of the water leaving the nozzle is reduced, reducing the effect reach of the stream.
...
\begin{figure}[H]
	...
	\hl{NEED FIGURE TO COMPARE TO HALF BALE AT SAME FLOW \& PRESSURE}
	\subcaption{Full Open}
	\end{minipage}
	\begin{minipage}[b]{0.45\textwidth}
	\centering
	% \includegraphics[width=0.95\textwidth]{}
	\hl{COULD NOT FIND FIGURE OF HALF BALE}
	\subcaption{Half Open}
	\end{minipage}
	\caption{Comparison of Distribution for Smooth Bore Varying Bale Position}
	\label{fig:Bale Position}
\end{figure}
