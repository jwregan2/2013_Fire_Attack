...
\end{tabular*}
\caption[Air Entrainment Comparison of Nozzle Movement Patterns]{Air entrainment for three nozzle movement patterns: `O', `Z', and `n' for a straight stream (left) and smooth bore stream (right).}
\label{fig:1_5_Interior_Nozzle_Movement_Comparison}
\end{figure}

An experiment was conducted to quantify potential differences in air entrainment when a given smooth bore nozzle with flow rate of 210~gpm and pressure of 50~psi was rotated with a `O' pattern at different rotation speeds. Using a metronome, the `O' pattern was applied at 50, 100, and 150 revolutions per minute. This test was conducted at the fixed interior setback distance of 12~ft. from the tip of the nozzle to the ventilation opening.
...
As shown in the figure above, an increase in the rotation speed while applying a specified pattern yielded an increase in the air entrainment seen within the stream.
