...
In the following subsections, the temperatures measured by thermocouple array in the fire room are used to determine the time to ``knock back'' the fire and are compared between experiments in five different groups based on fire attack method. Three of the groups contain experiments that used an interior fire attack method, and two of the sets contain experiments in which the first suppression event was an exterior fire attack. The differentiating factor between experiments within the three groups based on an interior fire attack method is the ventilation profile. The main difference between experiments in the other two groups is the ventilation profile and the fire attack method used after the initial exterior suppression event.
...
The front door being closed for the first two experiments, 18 and 20 yields no change to the flow at the door from water application. In the third experiment, 21, the front door was open when water was applied. The flow at the top middle probe went from approximately 0 to an inflow of 1.5~m/s. This indicates more air being drawn in when water flow occured in the window. Following the water flow the probes were moved to allow for the attack crew to enter which is the return to 0 for several seconds. When the probes were put back in place the gas velocities were of the same magnitude as they were before they were moved. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_19/Bedroom_1_Door}
\includegraphics[width=0.45\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_18/Bedroom_1_Door}
\caption[Gas Velocities - Single Room of Fire - Exterior Fog]{Gas velocities at the Start of Hall for exterior water application with a fog stream vs. a straight stream one room of fire and a single window vent. The front door is closed. The blue shaded areas represent where water was flowing from the hose stream. The grey area indicates data that is not accurate due to water hitting the velocity probes.}
\label{fig:push_fire_dead_end_fog}
\end{figure}

The fog stream in the window is capable of pushing products of combustion into the hallway, however further down the hallway at the start of the hall, the fog suppression has an oppsite effect, where it stops the flow into the living room. The straigh stream hits the velocity probes at the door, however at the start of the hallway the results of a fog stream and a staight stream have the same effect of contraction and cooling, stoping the flow down the hall. 

\section{Impact of ``Whip'' on Regrowth}
During all experiments that used an exterior fire attack method, an interior attack was performed after the exterior suppression event(s) to completely extinguish the fire. In nearly every one of these experiments, the temperature measured 7~ft above the fire room floor started to increase in the time between the end of exterior suppression and start of interior suppression, indicating the exterior attack hadn't fully extinguished the fire and that it was beginning to regrow. However, the rate and magnitude of the temperature increase was different between experiments.  
...
\subsection{No Vent}
To evaluate the impact of door control experiment 6 which included door control is compared to experiment 3, where no door control was provided. Figure \ref{fig:door_control_no_vent} below shows the fire room, gas velocity at the start of the hallway and the heat flux to the floor for both experiments. 
