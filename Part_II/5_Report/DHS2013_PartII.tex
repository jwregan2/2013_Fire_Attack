...
\subsection{Interior}
This section will evaluate the impact of different advancement types during an interior attack on the safety of fire suppression personnel.  
...
The were two methods of approach to the fire. The first titled `flow and move' involved flowing the hose line while moving down the hallway towards the fire room. The second titled `shutdown and move' involved flowing the nozzle from a fixed position, shutting it down, moving up to the next position, flowing again, shutting it down and moving to the next position until the crew reached the fire room. This section will compare these two tactics as they relate to temperatures in the approach path and temperatures in the fire room for the three ventilation cases tested. 

\subsection{Single Bedroom Fire - No Ventilation}
The single room fires with no ventilation as illustrated in Figure \ref{fig:Vent_Profile-No_Vent} were interior attacks with no ventilation opposite. Figure \ref{fig:Flow_vs_Shut_Single_No_Vent} shows the comparison between the flow and move tactic and shutdown and move tactic. With no ventilation provided opposite the attack crew, the shutdown and move tactic results in temperatures rebounding in the upper level of the hallway when the line is shutdown as the attack crew makes their way down the hallway. This is not seen in the flow and move tactic as the hose stream is continuously cooling as the crew makes their way down the hallway. Additionally the impact of the stream at the end of the hall is not immediately realized, a delay occurs in the cooling effect of the stream.  

\begin{figure}[H]
\centering
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_2/End_Hall}
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_6/End_Hall}
\caption[Single Room - No Vent - Flow \& Move vs. Shutdown \& Move]{End of Hallway Temperatures for the two attack methods used in the single room of fire with no ventilation. The right shows a flow and move tactic and the left shows the shutdown and move tactic.}
\label{fig:Flow_vs_Shut_Single_No_Vent}
\end{figure}

When ventilation is provided opposite the attack crew as the cooling has a more immediate impact as seen in Figure \ref{fig:Flow_vs_Shut_Single_Vent}. This is most likely due to the additional ventilation opposite the crew, resulting in hot gases being directed towards the open vent. The same temperature rebounding occurs in the shutdown and move case, it is again not seen in the flow and move case. 

\begin{figure}[H]
\centering
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_7/End_Hall}
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_8/End_Hall}
\caption[Single Room - Window Vent Oppsite - Flow \& Move vs. Shutdown \& Move]{End of Hallway Temperatures for the two attack methods used in the single room of fire with ventilation oppsite the attach crew. The right shows a flow and move tactic and the left shows the shutdown and move tactic.}
\label{fig:Flow_vs_Shut_Single_Vent}
\end{figure}

With two rooms of fire the same variations between the tactics are seen as shown in Figure \ref{fig:Flow_vs_Shut_Two_Vent}. The temperature rebound when the line is shutdown in a similar manor as the other ventilation cases. The cooling impact of the hose stream is immediately realized as the hose is directed down the hallway.  

\begin{figure}[H]
\centering
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_13/End_Hall}
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Tactical_Considerations/Flow_vs_Shutdown/Experiment_14/End_Hall}
\caption[Two Room - Two Vents Oppsite - Flow \& Move vs. Shutdown \& Move]{End of Hallway Temperatures for the two attack methods used in the two rooms of fire with two windows of ventilation oppsite the attack crew. The right shows a flow and move tactic and the left shows the shutdown and move tactic.}
