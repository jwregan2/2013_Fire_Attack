...
\begin{figure}[H]
\centering
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Thermal_Class/Experiment_1__408_Thermal_Exposure} 
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Thermal_Class/Experiment_12__324_Thermal_Exposure}
\includegraphics[width=0.495\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Thermal_Class/Experiment_17__312_Thermal_Exposure} 
...
\begin{enumerate}{}
\item Exterior Flow [min:sec] - Total flow time for exterior attack, start of intial knock down to end of respective whip. 
...
\item Interior Flow [min:sec] - Total flow time for interior attack, starting at intial flow (beginning of the hallway) and ending at final flow (end of hallway). \\
\item Interior Travel [min:sec] - Total suppression crew movement time interior of the stucture, starting at the beginning of the hallway location ending at the end of the hallway location. \\
\item Victim 4 [min:sec] - Total time the suppression crew took to reach the location of Victim 4, from intial attack to location of Victim 4. \\
\item Victim 1 [min:sec] - Total time the suppression crew took to reach the locaiton of Victim 1, from intial attack to location of Victim 1. \\
\item Tacic Time [min:sec] - Total time from intial intervention (exterior or interior flow) to final intervention (final flow or end of hallway location). \\
\end{enumerate}
...
\begin{table} [H]
\centering
\caption{Average Tatic Times for Interior Shutdown and Move Attack (Time~min:sec)}
...
\begin{table} [H]
\centering
\caption{Average Tatic Times for Interior Flow and Move Attack (Time~min:sec)}
\begin{tabular}{|C{0.05\textwidth}|C{0.15\textwidth}|C{0.10\textwidth}|C{0.10\textwidth}|C{0.10\textwidth}|C{0.10\textwidth}|}
\hline
...
\begin{table} [H]
\centering
\caption{Average Tatic Times for Transitional Attack (Time~min:sec)}
...
A transitional attack on the fire venting from the window followed by immediate advancement to the interior may be the most effective method to knock back, confine and suppress the fire. If this tactic is chosen, further considerations should include water mapping and air entrainment as identified by UL FSRI \cite{Weinchenk_watermapping} \cite{Weinchenk_airentrainment}; applying water off the ceiling at a steep angle, using a solid or straight stream in a fixed position (Section \ref{tc:water_impact_flowpath}); the importance of following the initial flow with immediate advancement to the interior (Section \ref{tc:speed_enemy_regrowth}); water application did not produce steam expansion or increased moisture for victims (Sections \ref{tc:steam_expansion} \& \ref{tc:steam_is_biproduct_of_combustion}; and the fact that you don't need a considerable amount of water to knock back the fire (Section \ref{tc:water_where_it_needs_to_go}).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.32\textwidth]{../0_Images/Tactical_Considerations/Transitional_Attack/Suppression}
\includegraphics[width=0.32\textwidth]{../0_Images/Tactical_Considerations/Transitional_Attack/1_Sec}
\includegraphics[width=0.32\textwidth]{../0_Images/Tactical_Considerations/Transitional_Attack/2_Sec}
\includegraphics[width=0.32\textwidth]{../0_Images/Tactical_Considerations/Transitional_Attack/3_Sec}
...
For a room and contents fire, the most important timing piece is the initial application of water into the compartment, prior to the fire extending outside the compartment. If that can be achieved faster by deploying the line to side `C' then that is the most effective tactical choice. If it can be acheived faster by conducting an interior attack through the front door, then that is the most effective tactical choice. 

Adding in items such as vehicles, fences, porches, adjacent structures, changes in grade, landscaping can change significantly impact the deployment of the line. Additionally, deploying the line for an interior attack can encounter obstacles such as a locked front door(s), hording conditions, devided up interiors, or complex floor plans which can delay the advancement to the fire compartment. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Thermal_Class/Experiment_12__324_Thermal_Exposure}
\caption[Conditions outside the fire compartment - Single Room with Vent]{Temperature and Heat flux during the 60~second period where interior suppression would have occurred in the delayed intervention cases as compared to the Thermal Classes Proposed by Utech, for a single bedroom fire with a window vent. The lightly shaded areas represent areas outside of Utech's thermal classes but still a relatively similar thermal hazard. (Experiment 12)}
...
Throughout the full-scale fire experiments, thermal imagers were used in both a static and dynamic manner. Sixteen video feeds, including two thermal imaging views, were captured and recorded for each of the experiments. \hl{One thermal view was from the interior looking down the hallway towards the fire room and one was from the exterior looking at the front side of the structure encompassing both the front door and window to the fire room within the field of view.} \textit{\hl{Wasn't it just inside the front door looking at the bookshelf?}} Additionally, a hand-held thermal imager was utilized by the firefighting crews engaged during the experiment for both safety purposes and to ensure complete knockdown and extinguishment. The thermal imagers utilized during this study were standard equipment readily available for purchase and use by the fire service worldwide. 

In post processing \hl{reviewing?} the experiment video, several of the limitations of thermal imaging use during firefighting operations were confirmed. As outlined within \textit{NFPA 1408: Standard for Training Fire Service Personnel in the Operation, Care, Use, and Maintenance of Thermal Imagers}, all personnel using thermal imagers must understand their use and limitations \cite{NFPA1408}. Thermal imagers are designed to detect radiant thermal energy emitted from solid surfaces, particulates, and some gases utilizing a constant emissivity value. Emissivity is defined as the ratio of the energy radiated from a material`s surface to that of a blackbody, or perfect emitter, at the same temperature. The emissivity of the surfaces and gases radiating energy during a fire affect the thermal imager`s ability to determine the actual temperature and can be misleading to the firefighters utilizing the device. As the emissivity value of radiating objects within a fire diverge from the imager`s constant value, the temperature readout becomes more and more inaccurate. 

Furthermore, the fire environment within a structure is always changing, especially with ongoing fire department interventions. There are situations in which a thermal imager can be ``looking through'' smoke to a surface beyond which could be at a significantly different temperature than that of the gases. \texthl{Could it also be that the imager is picking up the cooler particles of soot that are infront of the hot gasses? The mid hall temperatire at what looks to be 5' is $\approx$250F and the 2' is $\approx$150F.} With regards to the fire experiments, this was evident through looking at the thermal imaging view from the camera located on the interior of the structure looking down the hallway towards the fire room and comparing the temperature readout with the thermocouple temperatures located in the same vicinity as seen in figure \ref{fig:IR_temp_comparison}.

\begin{figure}[H]
\centering
\includegraphics[width=.48\textwidth]{../0_Images/Tactical_Considerations/Thermal_Imager/hallIR_exp16.png}
\includegraphics[width=.48\textwidth]{../0_Images/Tactical_Considerations/Thermal_Imager/endhall_exp16.png}
\caption[IR Temperature Comparison]{Comparison of Thermal Imaging temperature readout to End Hall thermocouple temperatures.}
\label{fig:IR_temp_comparison}
\end{figure}

As seen in the figure above, a screenshot capture of the thermal imaging view from inside the structure shows the end of the hallway at what appears to be a temperature of 136 degrees Farenheit at 5 minutes and 30 seconds into the test. The end hall thermocouple temperatures in the same vicinity are showing temperatures from floor to ceiling of well above 500 degrees Farenheit at the same time frame. This can be misleading to firefighters operating within the structure who may be making decisions on advancement and suppression based upon the temperature readout from the thermal imager.

The following is a list of recommendations in the use of thermal imaging cameras for structural firefighting operations:
\hl{Is this how we are reccomending using them?}
\begin{itemize}
	\item{Thermal imagers are unreliable termometers.}
	\item{Thermal imagers should be utilized to look for thermal contrasts, movement of heated gases, and heat signitures.}
	\item{Thermal imagers can be utilized to look above and in front to see radiant heat which may help in guiding the attack crews to the seat of the fire.}
	\item{Thermal imagers can be utilized to aid in the search for victims within the structure keeping in mind that the victim may appear ``white'' in a room in which the victim would be the hottest object and ``black'' in a room in which the victim is the coolest. The victim may also not be visible at all in which the victim is the same temperature as the surrounding objects.}
	\item{The field of view is smaller than apparent and objects may be overlooked if this is not considered.}
\end{itemize}
...
\chapter{Definitions}
\begin{description}
\item[Knock Back] A state of partial fire extinguishment wihch will allow for fire regrow in a short period of time without additional intervention.
\item[Knock Down] A state of partial fire extinguishment that is close to full extinguishment and where regrowth is unlikely.
\item[Whip] An addition form of exterior water application used in transitional attack to gain better hold time on the fire room. Typically the nozzle is positioned inside the window of the room and opened to a half bale or fog stream and moved in a rapid circular below the plane of the window sill for a period of time. 
\item[Half Bale] Using the broken stream of the nozzle to get water into the fire room above grade level. This is a secondary water application method for the exterior portion of transitional attack. 
\item[Transitional Attack] 
A Fire Attack where the application of water starts on the exterior and transitions to an interior application. \\
An offensive fire attack initiated by an exterior, indirect hand line operation into the fire compartment to initiate cooling while transitioning into interior direct fire attack in coordination with ventilation operations. 
\item[Defensive Attack]
...
An advance into the fire building by firefighters with hose lines or other extinguishing agents that are intended to overpower the fire.\\
...
\item[Exterior Attack]
\item[Interior Attack] The assignment of a team of fire fighters to enter a structure and attempt fire suppression.\\
\item[Fire Attack]
\item[Blitz Attack] To aggressively attack a fire from the exterior with a large diameter (2-1/2 inch [65 mm] or larger) fire stream.
\item[Fire] 
A rapid oxidation process, which is a chemical reaction resulting in the evolution of light and heat in carrying intensities.\\
...
A fire that has sufficient oxygen for fire growth but has a limited amount of furl available for burning.\\
A fire that has a heat release rate that is controlled by the material burning.\\
\item[Jargon]The specialized or technical language of a trade, profession, or similar group.
\item[Emmisitivity]The ratio of the energy radiated from a material‘s surface to that of a blackbody, or perfect emitter, at the same temperature.
\end{description}
