...
In the following section, the time to ``knock back'' the fire is compared between the experiments in five different groups that were formed based on fire attack method. Three of the sets involved an interior fire attack method, and two sets involved an exterior fire attack method. The differentiating factor between the experiments within each group involving an interior fire attack method is the ventilation profile. 

[EXTERIOR ATTACK DIFFERENCES] The following subsections use the temperature data from experiments to determine how ventilation profile affects the ability of the different attack methods to knock back the fire. 

\subsection{Flow \& Move with Solid Stream}
The first set of experiments is composed of Experiments~2, 7, and 13, which contained the `No Vent', `Single Vent', and `Two Vent' ventilation profile, respectively. The `flow and move' attack method with a solid stream from a smooth bore nozzle was the primary attack method used during each experiment. Figure~\ref{fig:knockback_int_1} contains a plot of the Bedroom~1 (fire room) temperatures from six seconds before the interior suppression event to one minute after the event for each experiment.
...
\begin{figure}[H]
	...
	\caption[Flow \& Move with Solid Stream --- Bedroom~1 Temperatures]{Bedroom~1 temperatures six seconds before the interior suppression event to one minute after the event during Experiments~2 (top left), 7 (top right), and 13 (bottom). The `flow and move' method with a solid stream from a smooth bore nozzle was the primary attack method used during each experiments.}
	\label{fig:knockback_int_1}
\end{figure}

Looking at the plots, the Bedroom~1 temperatures before the suppression event for Experiment~2 are indicative of temperatures before flashover (i.e., a significant temperature distribution is present), while the Bedroom~1 temperatures at the same time during Experiments~7 and 13 are indicative of a room in the flashover state. The `Single Vent' and `Two Vent' configurations used during Experiments~7 and 13 provided a steady source of oxygen to the fire room through the Bedroom~1 window unlike the `No Vent' configuration used during Experiment~2. 

Despite the difference in the temperature distribution before suppression, the Bedroom~1 temperatures behaved similarly after suppression began in each experiment; the measured temperatures in the fire room fell below 400~$^\circ$F within 30 seconds after the start of suppression and eventually converged to a value near 200~$^\circ$F by 50 seconds after the start of suppression. 

\FloatBarrier

\subsection{Shutdown \& Move with Solid Stream}
The second set of experiments is composed of Experiments~6, 8, and 14, which contained the `No Vent', `Single Vent', and `Two Vent' ventilation profile, respectively. The `shutdown and move' attack method with a solid stream from a smooth bore nozzle was the primary attack method used during each experiment. Figure~\ref{fig:knockback_int_2} contains a plot of the Bedroom~1 (fire room) temperatures from six seconds before the interior suppression event to one minute after the event for each experiment.
...
\begin{figure}[H]
	...
	\caption[Shutdown \& Move with Solid Stream --- Bedroom~1 Temperatures]{Bedroom~1 temperatures six seconds before the interior suppression event to one minute after the event during Experiments~6 (top left), 8 (top right), and 14 (bottom). The `shutdown and move' method with a solid stream from a smooth bore nozzle was the primary attack method used during each experiments.}
	\label{fig:knockback_int_2}
\end{figure}

As in the previous comparison of experiments, the temperature distribution before suppression for the experiment with the `No Vent' configuration, Experiment~6, was significantly larger than the temperature distribution present at the same time during the other two experiments. Again, due to the lack of a steady oxygen source to the fire room in the `No Vent' scenario, the room wasn't able to transition to the flashover stage like it did in Experiments~8 and 14, which contained the `Single Vent' and `Two Vent' configurations. 

Despite the difference in the initial temperature distribution, the Bedroom~1 temperatures decreased at a similar rate in each of the three experiments; the temperatures only begin to significantly decrease after the third period of water flow (around 30 seconds after the initial suppression event), when the attack crew was at the end of the hall near the doorway of Bedroom~1. For Experiment~6, all temperatures dropped below 400~$^\circ$F by the end of the third instance of water flow. The same may also be true for the temperatures during the same period for Experiment~8. Based on the sharp rise in temperature measured at 1~ft, 3~ft, and 5~ft above the floor and the erratic behavior that occurred in the temperature data at the same locations later in the experiment (after the period shown in the plot), it is likely that the thermocouples at the three locations were hit by water during the third suppression event, which adversely affected the temperature data. However, the temperature 7~ft above the floor declined at a rate similar to Experiment~6 and decreased to approximately 200~$^\circ$F by the end of the third instance of water flow like Experiment~6. Note, although temperatures began to sharply decline during the third instance of water flow during Experiment~14, the temperatures didn't drop below 400~$^\circ$F until the next period of water flow around the 45 second mark. The longer duration of Bedroom~1 temperature decline might be due to the fact that the `Two Vent' setup used during Experiment~14 contained two fire rooms, so the attack crew divided the water application between two rooms instead of using all the water to extinguish the fire in Bedroom~1. [CHECK VIDEO TO CONFIRM]  

\FloatBarrier

\subsection{Flow \& Move with Narrow Fog}
The third set of experiments is composed of Experiments~11 and 16, which contained the `Single Vent' and `Two Vent' ventilation profile, respectively. The `flow and move' attack method with a narrow fog stream from a combination nozzle was the primary attack method used during both experiments. Figure~\ref{fig:knockback_int_3} contains a plot of the Bedroom~1 (fire room) temperatures from six seconds before the interior suppression event to one minute after the event for each experiment.
...
\begin{figure}[H]
	...
	\caption[Flow \& Move with Narrow Fog --- Bedroom~1 Temperatures]{Bedroom~1 temperatures six seconds before the interior suppression event to one minute after the event during Experiments~11 (left) and 16 (right). The `flow and move' method with a narrow fog stream from a combination nozzle was the primary attack method used during each experiment.}
	\label{fig:knockback_int_3}
\end{figure}

Looking at the two plots, the attack crew flowing water in a narrow fog stream for less than 30 seconds while moving down the hall during Experiment~11 caused all Bedroom~1 temperatures to decrease to under 200~$^\circ$F. The same intervention during Experiment~16 caused three of the four Bedroom~1 temperatures to decrease to values around 200~$^\circ$F. The faster rate of Bedroom~1 temperature decline that occurred during Experiment~11 is a result of the `Single Vent' configuration having only one fire room, while the `Two Vent' configuration used during Experiment~16 had two fire rooms.

\FloatBarrier

\subsection{Initial Exterior Attack at Bedroom~1 Window}
The next set of experiments is composed of Experiments~18, 20, 22, and 24. The initial suppression event for all four experiments was an exterior attack that involved flowing water through the window of Bedroom~1 into the fire room. Water was applied in a solid stream pattern from a smooth bore nozzle for all experiments except Experiment~24, which used a combination nozzle set to a straight stream pattern to apply water. Experiments~18 and 20 contained the `Single Vent' ventilation profile, and Experiments~22 and 24 contained the `Two Vent' ventilation profile. Figure~\ref{fig:knockback_ext_1} contains a plot of the Bedroom~1 (fire room) temperatures measured during each experiment for the time period from four seconds before the initial suppression event to 24 seconds after the event.
...
\begin{figure}[H]
	...
	\caption[Initial Exterior Attack --- Bedroom~1 Temperatures]{Bedroom~1 temperatures from four seconds before the initial exterior suppression event to 24 seconds after the event during Experiments 18 (top left), 20 (top right), 22 (bottom left), and 24 (bottom right). }
	\label{fig:knockback_ext_1}
\end{figure}

Looking at the four plots, the initial exterior attack significantly reduced the temperatures in Bedroom~1 during all experiments. Additionally, the temperature distribution became inverted (i.e., the temperature \textit{decreased} as the measurement location increased in height) as temperatures declined and remained inverted along the duration displayed on the plots. 

The value to which the temperatures decreased is dependent upon the experimental configuration. During experiments containing the `Single Vent' setup (Experiments~18 and 20), the largest measured temperature after the suppression event was below 400~$^\circ$F, while the highest temperature recorded after suppression was between 800~$^\circ$F and 900~$^\circ$F for experiments containing the `Two Vent' setup (Experiments~22 and 24). The reason for this discrepancy can be at least partially attributed to the fact that the fire in the `Two Vent' configuration was larger than the fire in the `Single Vent' configuration. 

\FloatBarrier

\subsection{Exterior Attack at Bedroom~1 Window followed by Second Attack}
The last set of experiments is composed of Experiments~22 and 24, which both contained the `Two Vent' ventilation profile. The initial suppression event for each experiment was an exterior attack that involved flowing water through the window of Bedroom~1 into the fire room. Water was applied in a solid stream from a smooth bore nozzle during Experiment~22 and in a straight stream from a combination nozzle during Experiment~24. 

The interventions after the initial exterior suppression event differ between the two experiments. After the initial exterior attack in Experiment~22, the attack crew performed an interior attack that resembled the `shutdown and move' attack method. After the initial attack in Experiment~24, the attack crew moved to the window of Bedroom~2 and performed an additional exterior attack by flowing water into Bedroom~2 (the other fire room) through the window. Temperatures measured during the period from six seconds before the initial exterior suppression event to 95 seconds after the event are compared between the two experiments. 

Figure~\ref{fig:knockback_ext_2} contains a plot of the Bedroom~1 temperatures and a plot of the Bedroom~2 temperatures during this time period in Experiments~22 and 24. Plots of the temperatures at the end of the hallway and the temperatures in Bedroom~4 during the stated duration for each experiment are provided in Figure~\ref{fig:knockback_ext_3}.
...
Based on the plots of the temperatures within the two fire rooms (Figure~\ref{fig:knockback_ext_2}), the initial exterior attack during each experiment had a very similar effect on the fire environments in Bedroom~1 and Bedroom~2. 

After the initial attack, the temperatures in Bedroom~1 significantly decreased and became inverted. After the initial suppression, the Bedroom~1 temperatures during both experiments decreased according to the following: the temperature at 7~ft above the floor decreased to a value around 200~$^\circ$F; the temperature at 5~ft decreased to a value between 400~$^\circ$F and 500~$^\circ$F; the temperature at 3~ft decreased to a value between 600~$^\circ$F and 700~$^\circ$F; and the temperature at 1~ft decreased to a value between 800~$^\circ$F and 900~$^\circ$F. Furthermore, despite the differences in attack methods after the exterior suppression at the Bedroom~1 window, the Bedroom~1 temperatures continue to decrease in a similar manner over the plotted duration for both experiments, suggesting the initial attack extinguished most of the fire in Bedroom~1.

Looking at the Bedroom~2 temperature plots presented in the same figure, the initial exterior attack at the Bedroom~1 window caused the temperatures in Bedroom~2 to decrease slightly in both experiments. However, the decrease was only temporary, and in each experiment, the temperatures increased and nearly returned to their original values in the time between the end of the initial exterior attack and the next intervention. An additional slight decrease in Bedroom~2 temperatures occurred during Experiment~22 after hall suppression began. Then, as the attack crew moved down the hall closer to Bedroom~2, the temperatures began to significantly decrease once the crew started to flow water. While the Bedroom~2 temperatures decreased at a significant rate in response to the interior attack during Experiment~22, the Bedroom~2 temperatures decreased at a faster rate during the exterior attack performed at the window of Bedroom~2 during Experiment~24.
...
The plots of temperatures measured at the end of the hall and in Bedroom~4 for Experiments~22 and 24 are very similar. The temperatures at the end of the hall slightly decreased in response to the initial exterior suppression event and nearly returned to their original values before the next intervention during each experiment. The temperatures at the end of the hall decreased significantly in response to the second suppression event performed in each experiment. However, they declined at a faster rate in response to the hall suppression event during the interior attack in Experiment~22. Additionally, after second exterior attack during Experiment~24, the temperatures began to increase back to their original value. The hallway temperatures decreased permanently after the crew executed an interior attack after the two exterior attacks.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_7/Front_Door_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_7/Start_Hall_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_7/Bedroom_1_Window_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_9/Front_Door_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_9/Start_Hall_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_9/Bedroom_1_Window_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_11/Front_Door_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_11/Start_Hall_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_11/Bedroom_1_Window_Flow}
\caption[Gas Velocities - Single Room of Fire - Interior - Flow and Move]{Gas velocities at the Front Door (left column), Start of Hall (center column), and Fire Room Window (right column) for the flow and move interior attacks with one room of fire and a single window vent. Top row is Experiment 7, middle row is Experiment 9, and the bottom row is experiment 11. The blue shaded areas represent where water was flowing from the hose stream.}
...
At the start of the hall, the initial flow of water decreased the velocity the products of combustion were exiting out the top of the door from 5~m/s to almost zero. The inflow in the hall also decreased, indicating the exhange of gasses from the entrance to the hallway and the living room almost zero. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_8/Front_Door_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_8/Start_Hall_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_8/Bedroom_1_Window_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_10/Front_Door_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_10/Start_Hall_Flow}
\includegraphics[width=0.30\textwidth]{../0_Images/Script_Figures/Experiment_Analysis/Pushing_Fire/Experiment_10/Bedroom_1_Window_Flow}
\caption[Gas Velocities - Single Room of Fire - Interior - Shutdown and Move]{Gas velocities at the Front Door (left column), Start of Hall (center column), and Fire Room Window (right column) for the shutdown and move interior attacks with one room of fire and a single window vent. To top row is Experiment 8, and the bottom row is experiment 10. The blue shaded areas represent where water was flowing from the hose stream}
...
The flow out the bedroom window increased the first time the line was open, but subsubsaquent water applications did not produce the same changes. Once the line reached the fire room, the water stream hit the velocity probes, resulting in inaccurate readings. 

During interior suppression it is aparent water application has the potential to impact the flow of products of combustion. Applying water down the hallway, changed the velocity to be all in the direction of water application. Once the line is shutdown the flow returns to the conditions prior to application. If the fire is knocked down (temperatures reducted), the magnitue of the velocity changes, however the direction remains the same. The flow and move case was not as effective at maintaining the directional change of gas flow, potentially due to the difficulty in maintaining the nozzle pattern while moving down the hallway. The shutdown and move case changed the velocity each time the nozzle was opened, potentially because the attack crew was able to maintain a better application patern in a fixed position.
...
Figure \ref{fig:push_fire_exterior} shows the flows at the front door, start of the hallway, and bedroom window for the single room of fire and single open window ventilation cases where exterior. For the top row and middle row, experiments 18 and 20 respectivily, the front door was closed until after the water was applied through the window. The bottom row, experiment 21 had the front door open at the time of the application. 
...
\caption[Gas Velocities - Single Room of Fire - Exterior]{Gas velocities at the Front Door (left column), Start of Hall (center column), and Fire Room Window (right column) for exterior water application with one room of fire and a single window vent. The top row is Experiment 18, the middle row is experiment 20 and the bottom row is experiment 21. The blue shaded areas represent where water was flowing from the hose stream}
\label{fig:push_fire_exterior}
\end{figure}

The front door being closed for the first two experiments, 18 and 20 yields no change to the flow at the door from water application. In the third experiment, 21, the front door was open when water was applied. The flow at the top middle probe went from approximetly 0 to an inflow of 1.5~m/s. This indicates more air being drawn in when water flow occured in the window. Following the water flow the probes were moved to allow for the attack crew to enter which is the return to 0 for several seconds. When the probes were put back in place the gas velocities were of the same magnitude as they were before they were moved. 

At the start of the hallway the application of water decreased the velocity of the gasses at the top of the hallway from 5~m/s to 2.5~m/s indicating less products of combustion were flowing towards the door immediatly following water application.
...
When the end of the hallway values are evaluated with the window values it becomes apparent that entrainment is not the predominent reason for inflow in the window. The lack of increased flow at the start of the hallway even when the front door is open creating a flow path, indicates the predominent mechinsim of flow change was due to the cooling gasses contracting. This is only appicable when a straight stream or solid stream are used. 

\hl{Making a figure of the fog vs. the SS with the window vent. Demonstrating its difficult to push against a dead end.}
\hl{Check Bedroom Door Probes}

Figure X shows the results when a 

During exterior suppresion it is also possible to alter the flow of products of combustion in the space. When a solid stream or straight stream is used, limiting the air entrainment, the 
