...
for exp in ['Experiment_15_Data', 'Experiment_19_Data']:
	...
	for vic in ['Vic 1', 'Vic 3']:
		...
		x_min = 0
		x_max = (all_exp_events[exp[:-5] +'_Events']['Time_Seconds'][-2] + 120)/60 
		...
		ax1.plot(all_skin_data[exp].index, all_skin_data[exp][vic + ' necrosis depth'], label='Necrosis Depth', lw=3, marker=next(plot_markers), markersize=10, markevery=mark )
		ax2.plot(all_skin_data[exp].index, all_skin_data[exp][vic + ', surface necrosis'], label='Surface Necrosis', lw=3, marker=next(plot_markers),  markersize=10, markevery=mark )
		...
		for e in all_exp_events[exp[:-4]+'Events'][events].dropna().index:
			if e not in ['Ignition', 'End Experiment', 'Burst Suppression', 'Suppression Crew Enters', 'Suppression Crew Entry', 'BR1 Window Open']:
		...
		ax3.set_xticklabels(EventLabel, fontsize=32, ha='left')
		...
		ax1.set_xlabel('Time (Minutes)', fontsize=42)
		ax1.set_ylabel('Necrosis Depth (mm)', fontsize=42)
		ax2.set_ylabel('Surface Necrosis (Dimensionless)', fontsize=42)
		plt.yticks(fontsize=32)
		# ax2.set_yticks(fontsize=28)
		plt.tight_layout()
		...
		if not os.path.exists(output_location + '/Skin_Necrosis/'  + '/'):
			os.makedirs(output_location + '/Skin_Necrosis/'  + '/')

		plt.savefig(output_location + '/Skin_Necrosis/' + '/' + exp + '_' + vic.replace(' ','_').replace(',','') + '.pdf')
		plt.close('all')
...
necrosis_table = pd.DataFrame({'Experiment':[], '5_Seconds_Prior':[], '60_Seconds_Post':[], 
								 'Max_Value':[], }).set_index('Experiment')

surface_necrosis_table = pd.DataFrame({'Experiment':[], '5_Seconds_Prior':[], '60_Seconds_Post':[], 
								 'Max_Value':[], }).set_index('Experiment')
...
for vic in ['Vic 1', 'Vic 3']:
	...
	for vent in vent_info:
		for exp in natsorted(vent_info[vent].dropna()):
			...
			# print (flow_time)
			# print (exp + ' Surface Necrosis at Water is ' + str(all_skin_data[exp][all_skin_data[exp].index > (flow_time-5)/60].iloc[0][vic + ', surface necrosis']))
			# print (exp + ' Necrosis Depth (mm) is ' + str(all_skin_data[exp][all_skin_data[exp].index > (flow_time-5)/60].iloc[0][vic + ' necrosis depth']))
			...
			necrosis_table.ix[exp, '60_Seconds_Post'] = all_skin_data[exp][all_skin_data[exp].index > (flow_time+60)/60].iloc[0][vic + ' necrosis depth']
			...
			surface_necrosis_table.ix[exp, '60_Seconds_Post'] = all_skin_data[exp][all_skin_data[exp].index > (flow_time+60)/60].iloc[0][vic + ', surface necrosis']
	...
	surface_necrosis_table = surface_necrosis_table.round(3)
	...
	necrosis_table = necrosis_table[['5_Seconds_Prior', '60_Seconds_Post', 'Max_Value',]]

	surface_necrosis_table = surface_necrosis_table[['5_Seconds_Prior', '60_Seconds_Post', 'Max_Value',]]
