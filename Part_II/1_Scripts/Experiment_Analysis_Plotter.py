...
for exp in exp_info.index:
	low_data = all_laser_data[exp]['Low'][all_laser_data[exp]['Low'] != 0.0]
	high_data = all_laser_data[exp]['High'][all_laser_data[exp]['High'] != 0.0]

	low_data.dropna()
	high_data.dropna()

	low, = plt.plot(low_data, label = exp.replace('_', '' ))
	high = plt.plot(high_data , color=low.get_color())
