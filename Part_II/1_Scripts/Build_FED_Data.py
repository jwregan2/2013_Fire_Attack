...
for exp in exp_des.index.values:
	...
	for victim_loc in Victim_Locations:
		...
		if victim_loc + 'HFS' in all_exp_data[exp]:
			FED_flux = (1/(16.7)/(all_exp_data[exp][victim_loc+'HFS']**1.33))*time_step
		...
		if victim_loc + 'TCV1' in all_exp_data[exp]:
			FED_conv = (1/(2e18 * all_exp_data[exp][victim_loc + 'TCV1']**-9.0403 + 1e8 * all_exp_data[exp][victim_loc + 'TCV1']**-3.10898))*time_step
...
pickle.dump(FED_Temp_Conv, open (output_location + 'FED_Temp_Flux.dict' , 'wb'))
print ('-------------- FED_Temp_Flux.dict dumped to ../Data/FED folder ------------------')

pickle.dump(FED_Temp_Flux, open (output_location + 'FED_Temp_Conv.dict' , 'wb'))
