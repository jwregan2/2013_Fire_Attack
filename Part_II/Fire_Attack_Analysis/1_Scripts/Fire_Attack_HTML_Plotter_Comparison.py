...
output_location_init = '../0_Images/Results/HTML/Comparisons'

info_file = '../3_Info/Description_of_Experiments.csv'

# Read in channel list
channel_list = pd.read_csv(channel_location+'Channels_For_Comparison.csv')
	...
	for channel in channel_groups.get_group(group).index.values:
		...
		for f in os.listdir(data_location):
			if f.endswith('.csv'):
				...
				output_location = output_location_init +'Comparison'+'/'
