...
info_file = 'Description_of_Experiments_2.csv'
...
for f in os.listdir(data_dir):
	if f.endswith('.csv'):
		...
		ax.text(15, 4.5, 7,Vents, horizontalalignment='left', verticalalignment='bottom')
		...
		# plt.savefig('Figures_2/' + Test_Name.replace('/','_').replace('"','in') + Nozzle_Setting + Nozzle_Position + Nozzle_Pattern + '.png')
		plt.savefig('Figures_2/' + f[:-4] + '_' + Test_Name.replace('/','_').replace('"','in').replace(' ','_') + '.png')
		plt.close('all')
